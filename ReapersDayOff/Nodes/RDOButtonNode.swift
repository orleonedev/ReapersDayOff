...
@objc protocol ButtonNodeResponderType: AnyObject {
    /// Responds to a button press.
    @objc func buttonTriggered(button: RDOButtonNode)
}
...
enum ButtonIdentifier: String {
    ...
    case scoreLeaderboard = "ScoreLeaderboard"
    ...
    static let allButtonIdentifiers: [ButtonIdentifier] = [
        .resume, .home, .game, .retry, .start, .settings, .about, .collection, .tutorial, .credits, .scoreLeaderboard, .leaderboards, .sound, .haptics
    ]
...
class RDOButtonNode: SKSpriteNode {
    // MARK: Properties
    ...
    var focusableNeighbors = [ControlInputDirection: RDOButtonNode]()
    ...
    override func copy(with zone: NSZone? = nil) -> Any {
        let newButton = super.copy(with: zone) as! RDOButtonNode
