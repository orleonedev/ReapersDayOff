...
//        behavior.addHuntReaperGoal(forScene: scene)
...
//        behavior.addFleeReaperGoal(forScene: scene)
    ...
    private func extrudedObstaclesContaining(point: SIMD2<Float>, inScene scene: RDOLevelScene) -> [GKPolygonObstacle] {
        ...
        */
        let extrusionRadius = Float(GameplayConfiguration.Enemy.pathfindingGraphBufferRadius) + 5
    ...
    private func addAvoidObstaclesGoal(forScene scene: RDOLevelScene) {
        setWeight(1.0, for: GKGoal(toAvoid: scene.polygonObstacles, maxPredictionTime: GameplayConfiguration.Enemy.maxPredictionTimeForObstacleAvoidance))
    }
...
//    }
    ...
    private func addFollowAndStayOnPathGoals(for path: GKPath) {
        ...
        setWeight(0.5, for: GKGoal(toFollow: path, maxPredictionTime: GameplayConfiguration.Enemy.maxPredictionTimeWhenFollowingPath, forward: true))
